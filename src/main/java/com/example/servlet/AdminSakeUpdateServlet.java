 ...
 * 관리자 사케 수정 서블릿
 */
@WebServlet("/admin/sake/update")
public class AdminSakeUpdateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (nameJa == null || nameJa.trim().isEmpty() || 
            ...
            response.sendRedirect(request.getContextPath() + "/admin/sake/form?sakeId=" + sakeIdParam);
            return;
        }
        ...
        try {
            ...
            boolean success = sakeDAO.update(sake);
            
            if (success) {
                response.sendRedirect(request.getContextPath() + "/admin/sake/list");
            } else {
                request.setAttribute("error", "사케 수정에 실패했습니다.");
                response.sendRedirect(request.getContextPath() + "/admin/sake/form?sakeId=" + sakeId);
            }
